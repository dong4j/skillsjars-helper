...
| 二期 · 导出与发布 | Skill 导出到 Agent 目录、manifest 写入、SkillsJars 发布前检查 | 计划中 |
| 二期 · Gradle 解析 | Gradle dependencies 扫描；通过同一 `SkillSourceScanner` 扩展点接入 | 计划中 |

一期已经把扩展接口和扫描器扩展点（`dev.dong4j.idea.skillsjars.helper.skillSourceScanner`）一次性预留，
未来的 Gradle 扫描器和第三方查询插件只需要实现 `SkillSourceScanner` / 调用 `SkillRegistry`，
不需要改动现有协调层。
