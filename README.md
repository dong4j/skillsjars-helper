...
- 能否一键导出到 Claude Code、Codex、Junie、Cursor、Gemini、Qoder 等 Agent 的 Skill 目录？
...
5. Tool Window 列表展示。
6. 双击预览 `SKILL.md`。
7. 右键导出到 `.claude/skills`、`.codex/skills`、`.junie/skills`、`.agents/skills`、`.cursor/skills`、`.gemini/skills`、`.qoder/skills` 或自定义目录。
8. 导出时生成 manifest。
9. 提供 Refresh 按钮。
...
| 二期 · 导出 | Skill 导出到 7 个 Agent 目录、manifest 写入、6 状态冲突机 (含 DUPLICATE_NAME) | 已完成 |
...
1. 在内嵌 IDE 中右键 skill 叶子节点，选 `Extract to ▸`，应能看到 7 个预设 Agent 目录 + Custom Directory。
...
8. 再次重启 IDE → 安装徽标依然显示，因为 `InstallationRegistryService` 会在启动时重扫 7 个预设目录的 manifest。

详细设计见 `docs/design.md`。

## 致谢

ToolWindow 中用于安装徽标的 6 个 Agent 品牌图标（Claude / Codex / Junie / Cursor / Gemini / Qoder）来自
